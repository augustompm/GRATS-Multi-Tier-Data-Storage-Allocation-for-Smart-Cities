...
def render(pareto_file=None, mcda_file=None, output='fig1_pareto_3d.pdf'):
    ...
    sc0 = axes[0].scatter(Q1, Q2, c=Q3, cmap='viridis', s=30, alpha=0.7, edgecolors='gray', linewidths=0.3)
    ...
    plt.colorbar(sc0, ax=axes[0], shrink=0.7).set_label(r'$Q_3$ (kg/mo)', fontsize=8)

    sc1 = axes[1].scatter(Q1, Q3, c=np.log10(Q2 + 1e-3), cmap='plasma', s=30, alpha=0.7, edgecolors='gray', linewidths=0.3)
    ...
    plt.colorbar(sc1, ax=axes[1], shrink=0.7).set_label(r'$\log_{10}(Q_2)$', fontsize=8)

    sc2 = axes[2].scatter(Q2, Q3, c=Q1, cmap='cividis', s=30, alpha=0.7, edgecolors='gray', linewidths=0.3)
    ...
    plt.colorbar(sc2, ax=axes[2], shrink=0.7).set_label(r'$Q_1$ (k\$/mo)', fontsize=8)

    for name, (q1, q2, q3) in picks.items():
        st = PROFILE_STYLE[name]
        axes[0].scatter([q1], [q2], c=st['color'], s=180, marker=st['marker'],
                        edgecolors='black', linewidths=1.2, label=st['label'], zorder=5)
        axes[1].scatter([q1], [q3], c=st['color'], s=180, marker=st['marker'],
                        edgecolors='black', linewidths=1.2, zorder=5)
        axes[2].scatter([q2], [q3], c=st['color'], s=180, marker=st['marker'],
                        edgecolors='black', linewidths=1.2, zorder=5)
